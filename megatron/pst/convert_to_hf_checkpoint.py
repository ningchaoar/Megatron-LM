...
# use python >= 3.7
# PYTHONPATH=/work python convert_to_hf_checkpoint.py <path>/model_optim_rng.pt <path>/sparse_model.pt 
...
def convert_megatron_checkpoint(args, input_state_dict, sparse_state_dict, config):
...
def main():
    ...
    parser.add_argument(
        "path_to_checkpoint",
        type=str,
        help="Path to the checkpoint file (direct .pt file)",
    )
    parser.add_argument(
        "path_to_sparse_checkpoint",
        type=str,
        help="Path to the sparse checkpoint file (direct .pt file)",
    )
    parser.add_argument(
        "--config_file",
        default="",
    ...
    sparse_state_dict = torch.load(args.path_to_sparse_checkpoint, map_location="cpu")
    sparse_state_dict = load_sparse_model(sparse_state_dict)
    ...
    # Convert.
    print("Converting")
    output_state_dict = convert_megatron_checkpoint(args, input_state_dict, sparse_state_dict, config)
    ...
    output_checkpoint_file = os.path.join(basename, "hf_pytorch_model.bin")
