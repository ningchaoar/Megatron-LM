...
def generate(args, model, tokenizer):
    ...
    # print("elapsed time per generation (ms): {}".format(elapsed_time))
    latency_per_token = elapsed_time / output_sequences.shape[1]
    # print("latency per token (ms): {}".format(latency_per_token))

    if args.latency_test:
        return None, latency_per_token
    ...
    return generated_sequences, latency_per_token
...
def main():
    ...
    parser.add_argument("--latency-test", action="store_true", help="Get the latency performance for generation")
